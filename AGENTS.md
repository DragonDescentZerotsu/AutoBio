...
- The screw-thread reimplementation now uses the paper's z-axis helix convention. In MuJoCo 3.3.0 it matches the original plugin's generated `model/scene/screw_test.xml` SDF mesh vertex/face counts, bounds, mesh positions, and mesh quaternions. Use `autobio/plugins/mjlab_sdf_thread/compare_screw_test.py` for behavior regression. Current default `GRADIENT_EPS=5e-5` 2000-step 3.3.0 comparison: `ctrl=1` original yaw/dz/force `0.5075/0.000172/1.8066`, reimplementation `0.6502/0.000236/1.6295`; `ctrl=-1` original `-1.2978/-0.000980/7.6780`, reimplementation `-1.0072/-0.000967/8.1992`. Contact dynamics still need closer tuning, especially yaw balance and MuJoCo 3.8.1 parity.
